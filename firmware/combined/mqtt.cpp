...
void publishTelemetry(const SensorData& data, const char* aqiClass, bool anomalyDetected) {
  ...
  if (!mqttClient.connected()) {
    stationBufferPush(sample);
    Serial.printf("[MQTT] Offline — buffered (%u held, %u lost)\n",
                  (unsigned)stationBufferCount(), (unsigned)stationBufferDropped());
    return;
  }
...
void mqttServiceBuffer() {
  if (!mqttClient.connected() || stationBufferCount() == 0) return;

  for (int i = 0; i < REPLAY_PER_PASS; i++) {
    StationSample sample;
    if (!stationBufferPeek(sample)) return;
    char buffer[1024];
    buildTelemetryJson(sample, buffer, sizeof(buffer));
  ...
  if (stationBufferCount() == 0) Serial.println("[MQTT] Buffered telemetry fully replayed");
}
...
void publishHealth() {
  ...
  doc["buffered"] = (uint32_t)stationBufferCount();
  doc["dropped"] = stationBufferDropped();

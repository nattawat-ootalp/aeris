...
- **No persistent MQTT subscriber on Render Free.** A Render free *web* service sleeps and can't hold a long-lived MQTT connection. So Station data uses **HiveMQ webhook → FastAPI HTTP** (`POST /webhook/telemetry`, HMAC-signed). A native HiveMQ→HTTP webhook is **not** available on the free plan, so that bridge is `ingestion/mqtt_bridge.py`: it runs on an always-on host (the station gateway or a local PC), never inside the Render web service. Until it was written, the endpoint existed with nothing calling it — the station published to the broker and the readings were silently never ingested, which showed up as a station that was powered on yet reported no data. Whoever operates a station has to keep that process running.
...
- **Prototype demo runs the portable path only.** The station path is built and unit-tested, but the single available ESP32-S3 is flashed with `firmware/portable`, so node `BKK-TRT-003` publishes nothing and its telemetry endpoint answers `"no recent data"` — the correct behaviour, not a failure. No station code, endpoint, or test was deleted; re-flashing a board with `firmware/station` restores the path. See the *Prototype scope* section of the README for the other two prototype-only conditions (debug-keystore APK, unrotated Supabase development credentials).
